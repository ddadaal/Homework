...
class Solution {
  ...
  bool findPath(TreeNode* root, TreeNode* target, vector<TreeNode*>& path) {
	...
	if (root == target) { // root == target，找到了这条道路，并对父节点告知找到道路
		return true;
	}
	
	if (findPath(root->left, target, path)) { // 从左边走可以找到道路，对父节点告知可以从这一节点找到道路
		return true;
	}

	if (findPath(root->right, target, path)) { // 从右边走可以找到道路，对父节点告知可以从这一节点找到道路
		return true;
	}
