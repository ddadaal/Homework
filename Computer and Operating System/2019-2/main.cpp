...
void printColored(const char *pointer)
{
	print(COLOR_RED);
	print(pointer);
	print(COLOR_RESET);
...
public:
	...
	virtual void printDetailedPart() = 0;
};
...
public:
	...
	void printItem(bool detail)
	{
		if (isDir)
		{
			printColored(name.c_str());
		}
		else
		{
	...
	void printDetailedPart() override
	{
		if (isDir)
		{
			int fileCount = 0, dirCount = 0;
			for (auto item : getSubItems())
			{
...
public:
	...
	void printDetailedPart() override
	{
		...
		{
			if (item->isDirectory() && !item->isSpecial)
			{
				dirCount++;
			}
			else
...
void lsRec(Item *dir, string parent, bool detail)
{
	print(parent.c_str());
	if (detail)
	{
		print(" ");
		dir->printDetailedPart();
	}
	println(":");
	auto subItems = dir->getSubItems();
	for (auto item : subItems)
	{
		item->printItem(detail);
		if (detail)
		{
			print("\n");
		}
	...
	for (auto item : subItems)
	{
		if (item->isDir && !item->isSpecial)
		{
			lsRec(item, parent + item->name + '/', detail);
		}
	}
}
...
void ls(string path, bool detail)
{
	normalizePath(path);
		...
		for (int i = 1; i < separated.size(); i++)
		{
			...
			if (separated[i][0] == '-')
			{
				for (int j = 1; j < separated[i].length(); j++)
				{
					...
					{
						throw "Unrecognized option. Only l is supported.";
					}
				}
			}
			else
